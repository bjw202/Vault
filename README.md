...
- **파일 하나씩 순차 처리**: 배치 처리하지 않는다. 각 파일 완료 시점에 `_compile_log.md`에 로그 한 줄이 찍히며, 이게 재시작 가능한 checkpoint다. 컨텍스트가 부족해지면 `/compile`을 다시 실행해 남은 파일만 이어서 처리
- **concept-first 순서**: 개념 추출 → 인덱스 대조 → 신규 concept 파일 먼저 생성 → source 작성 + 링크 삽입. 이 순서 덕분에 `[[link]]`가 박히는 시점에 타깃 파일이 이미 존재 (broken link 구조적 방지)
...
wiki 지식 그래프의 품질을 점검하고 수정한다. **자동 수정** vs **사용자 판단 요청** 이분법으로 동작:

- **자동 수정** (묻지 않음): 깨진 concept 링크 → 스텁 파일 생성, 중복 concept 병합, 인덱스 동기화, frontmatter 필드 누락 복구
- **사용자 판단 요청**: 고아 문서 삭제 여부, 원본 없는 source 처리, 오타 의심 링크 리네이밍
- 스캔 순서가 결과에 영향을 준다 — 스텁 생성 먼저, 고아 탐지 나중 (스텁이 고아로 잡히지 않도록)
- 상세는 `.claude/skills/graph-lint/SKILL.md` 참조
